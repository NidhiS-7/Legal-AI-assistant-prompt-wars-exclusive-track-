...
*(Update this section with the actual diffs once you deploy — a sample
entry is provided below as a template.)*
...
- Set `MAX_UPLOAD_MB` via environment variable to match the platform's
  request size limits.
- No code changes were required between local and deployed versions —
  the app reads configuration exclusively from environment/secrets,
  which was a deliberate design choice for portability (see `app.py::get_api_key`).
