...
- All internal/library exceptions are caught and converted into safe,
  ...
  shown in the UI.
- **Per-session AI request cap** (`MAX_AI_CALLS_PER_SESSION`, default 30):
  protects the shared free-tier quota from being exhausted by a runaway
  loop, accidental repeated clicks, or a single misbehaving session -
  see `app.py::call_ai`.
...
- **Document parsing is cached** (`app.py::_parse_document_cached`, via
  `st.cache_data`): Streamlit reruns the whole script on nearly every
  widget interaction, so without caching the same PDF/DOCX would be
  re-parsed repeatedly even though its bytes never changed.
- **AI responses are cached per session** (`app.py::call_ai`), keyed on a
  hash of the document text and task parameters: re-running the same
  task on the same document (e.g. switching modes and back) is free and
  instant instead of re-spending a Gemini call.
- **Long documents use map-reduce chunking** (`core/chunker.py` +
  `core/ai_client.py::_process_long_document`) rather than a single
  oversized request, keeping each individual API call fast and within
  comfortable context limits.
- Q&A intentionally is **not** cached, since the same question can
  legitimately produce a different answer depending on prior chat
  history - caching it would silently give stale or wrong answers.
...
36 unit tests cover document parsing (valid/invalid files, size limits,
encodings, PDF/DOCX/TXT extraction), text chunking (boundaries, overlap,
content preservation, edge cases), prompt construction (grounding rules,
disclaimers, all 6 task prompts), and the AI client itself (retry
behavior on rate limits, fail-fast on auth/not-found errors, map-reduce
chunking and merging, truncation) — the last group uses a mocked Gemini
SDK (`tests/test_ai_client.py`) so retry/error-handling logic is
verified deterministically without making real network calls.

---

## 9. Deployment (Streamlit Community Cloud — free, ~5 minutes)
...
## 10. Changes Made in the Deployed Version
...
- **Decorative emoji are marked `aria-hidden="true"`** (the title's
  ⚖️ and the disclaimer's ⚠️), so screen readers announce the meaningful
  text ("Legal Document Assistant", the disclaimer sentence) without also
  reading out "balance scale emoji" or "warning emoji" as if it were
  content.
