...
└──────────┬───────────┘
           │ prompt
┌──────────▼───────────┐
│  core/ai_client       │  Calls Anthropic Claude API, retries with
...
3. Each section is sent through `prompts.build_simplify_prompt()` and
   `LegalAIClient.simplify_document()` to Claude.
...
*(The model string is configurable via the `ANTHROPIC_MODEL` environment
variable if you want to point it at a different Claude model.)*
...
cp .env.example .env            # then add your ANTHROPIC_API_KEY into .env
...
4. Add an environment variable `ANTHROPIC_API_KEY` under the service's
   **Environment** tab.
5. Deploy.
...
- **Added `runtime.txt` pinning Python to `3.11`.** Streamlit Community
  Cloud's default image provisioned Python 3.14 at deploy time, and
  `pillow==10.4.0` (a transitive dependency pulled in by
  `streamlit==1.38.0`) has no prebuilt wheel for 3.14. The platform fell
  back to compiling Pillow from source and failed with `zlib` headers not
...
- No application code changes were required between local and deployed
  versions — the app reads configuration exclusively from
  environment/secrets, which was a deliberate design choice for
...
├── core/
│   ├── ai_client.py             # Claude API wrapper: retries, chunk aggregation
