...
def test_chunk_text_splits_long_document_on_paragraphs():
    ...
    assert len(chunks) > 1
    # every chunk should respect the max size (with small overlap tolerance)
    for chunk in chunks:
...
def test_chunk_text_handles_single_oversized_paragraph():
    # One giant paragraph with no double-newlines at all.
    text = "word " * 5000
    chunks = chunk_text(text, max_chars=500, overlap=50)
    assert len(chunks) > 1
    for chunk in chunks:
        assert len(chunk) <= 500 + 5  # small tolerance for strip()
